defmodule RadioappWeb.DefaultsControllerTest do
  use RadioappWeb.ConnCase

  alias Radioapp.Factory

  @tenant "sample"
  @prefix Triplex.to_prefix(@tenant)

  @create_attrs %{callsign: "some callsign", from_email: "some from_email", from_email_name: "some from_email_name", logo_path: "some logo_path", org_name: "some org_name", phone: "some phone", playout_url: "some playout_url", privacy_policy_url: "some privacy_policy_url", support_email: "some support_email", tos_url: "some tos_url", website_url: "some website_url"}
  @update_attrs %{callsign: "some updated callsign", from_email: "some updated from_email", from_email_name: "some updated from_email_name", logo_path: "some updated logo_path", org_name: "some updated org_name", phone: "some updated phone", playout_url: "some updated playout_url", privacy_policy_url: "some updated privacy_policy_url", support_email: "some updated support_email", tos_url: "some updated tos_url", website_url: "some updated website_url"}
  @invalid_attrs %{callsign: nil, from_email: nil, from_email_name: nil, logo_path: nil, org_name: nil, phone: nil, playout_url: nil, privacy_policy_url: nil, support_email: nil, tos_url: nil, website_url: nil}

  describe "manage defaults" do

    setup %{conn: conn} do
      user = Factory.insert(:user, roles: %{@tenant => "admin"})

      conn = log_in_user(conn, user)
      %{conn: conn, user: user}
    end

    test "lists all defaults", %{conn: conn} do
      conn = get(conn, ~p"/defaults")
      assert html_response(conn, 200) =~ "Listing Defaults"
    end

    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/defaults", defaults: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/defaults/#{id}"

      conn = get(conn, ~p"/defaults/#{id}")
      assert html_response(conn, 200) =~ "Defaults"
    end

    # no new function
    # test "renders errors when data is invalid", %{conn: conn} do
    #   conn = post(conn, ~p"/defaults", defaults: @invalid_attrs)
    #   assert html_response(conn, 200) =~ "New Defaults"
    # end

    test "renders form for editing chosen defaults", %{conn: conn} do
      defaults = Factory.insert(:defaults, [], prefix: @prefix)
      conn = get(conn, ~p"/defaults/#{defaults}/edit")
      assert html_response(conn, 200) =~ "Edit Defaults"
    end


    test "redirects when data is valid", %{conn: conn} do
      defaults = Factory.insert(:defaults, [], prefix: @prefix)
      conn = put(conn, ~p"/defaults/#{defaults}", defaults: @update_attrs)
      assert redirected_to(conn) == ~p"/defaults/#{defaults}"

      conn = get(conn, ~p"/defaults/#{defaults}")
      assert html_response(conn, 200) =~ "some updated callsign"
    end

    test "renders errors when defaults data is invalid", %{conn: conn} do
      defaults = Factory.insert(:defaults, [], prefix: @prefix)
      conn = put(conn, ~p"/defaults/#{defaults}", defaults: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Defaults"
    end

    test "deletes chosen defaults", %{conn: conn} do
      defaults = Factory.insert(:defaults, [], prefix: @prefix)
      conn = delete(conn, ~p"/defaults/#{defaults}")
      assert redirected_to(conn) == ~p"/defaults"

    end
  end

  defp create_defaults(_) do
    defaults = Factory.insert(:link, [], prefix: @prefix)
    %{defaults: defaults}
  end
end
