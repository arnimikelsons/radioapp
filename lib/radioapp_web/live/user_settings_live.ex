defmodule RadioappWeb.UserSettingsLive do
  ...
  def render(assigns) do
    ...
    <div class="max-w-2xl mx-auto">
      ...
      <.header class="pt-5">Change Password</.header>
  ...
  def handle_event("validate_name", params, socket) do
    ...
    name_changeset = Accounts.change_user_name(socket.assigns.current_user, user_params)

    {:noreply,
     socket
     |> assign(:name_changeset, Map.put(name_changeset, :action, :validate))
     |> assign(:current_password, password)}
  end

  def handle_event("update_name", %{"current_password" => password, "user" => user_params}, socket) do
    user = socket.assigns.current_user
    ...
    case Accounts.update_user_name(user, password, user_params) do
      {:ok, user} ->
        socket =
          ...
          |> assign(:name_changeset, Accounts.change_user_name(user, user_params))
