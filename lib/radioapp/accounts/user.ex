defmodule Radioapp.Accounts.User do
  use Ecto.Schema
  ...
  def registration_changeset(user, attrs, opts \\ []) do
    user
    |> cast(attrs, [:email, :password, :role, :full_name, :short_name, :roles])
...
@doc """
  ...
  def name_changeset(user, attrs) do
    user
    |> cast(attrs, [:full_name, :short_name])
    |> validate_required([:full_name, :short_name])
  end
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique(:email, Radioapp.Repo)
  ...
  def accept_changeset(user, attrs, opts \\ []) do
    now = NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second)
  ...
  """
  def valid_password?(%Radioapp.Accounts.User{hashed_password: hashed_password}, password)
